...
const ForgotPassword = () => {
  ...
  useEffect(() => {
    if (passwordData.newPassword) setPasswordValidity(validatePassword(passwordData.newPassword));
    setPasswordMatch( validateMatch(passwordData.newPassword, passwordData.confirmNewPassword));
    ...
    const submitResetPassword = async (evt) => {
        ...
        if (!passwordData.newPassword || !passwordData.confirmNewPassword) return handleFormMessage('Required fields are marked with a (*)');
        ...
        try {
            ...
            if (response.message === 'User\'s password reset successfully') {
              ...
              setTimeout(() => { navigate('/login', { state: { path: location.pathname } }) }, 2000);
            }
